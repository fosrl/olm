...
func (o *Olm) handleWgPeerAdd(msg websocket.WSMessage) {
	...
	// Trigger immediate hole punch attempt so that if
	// the peer decides to relay we have already punched
	// close to when we need it
...
func (o *Olm) handleWgPeerUnrelay(msg websocket.WSMessage) {
	...
	// Update HTTP server to mark this peer as not using relay
	o.apiServer.UpdatePeerRelayStatus(relayData.SiteId, relayData.Endpoint, false)

	_ = o.peerManager.UnRelayPeer(relayData.SiteId, primaryRelay)
}
