...
func (o *Olm) handleConnect(msg websocket.WSMessage) {
	...
	if config.FileDescriptorTun == 0 {
		if realInterfaceName, err2 := o.tdev.Name(); err2 == nil {
	...
	o.dnsProxy, err = dns.NewDNSProxy(o.tdev, o.middleDev, config.MTU, wgData.UtilitySubnet, config.UpstreamDNS, config.TunnelDNS, interfaceIP)
	if err != nil {
		logger.Error("Failed to create DNS proxy: %v", err)
	}

	if err = network.ConfigureInterface(o.tunnelConfig.InterfaceName, wgData.TunnelIP, config.MTU); err != nil {
	...
	if config.OverrideDNS {
		...
		if err := dnsOverride.SetupDNSOverride(o.tunnelConfig.InterfaceName, o.dnsProxy); err != nil {
